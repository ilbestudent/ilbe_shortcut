...
function movePage(relPos) {
  try{
    ...
    console.log("exceptio caught @ movePage", err);
  }
}
...
function moveArticle(relPos, refresh) {
  ...
  {
    console.log("exception caught @ moveArticle", err);
  }
}

function wasMovingArticle(){
  try{
    ...
    console.log("exception caught @ wasMovingArticle", err);
  }
}

function showUpdateNotification(myLocalStorage){
  try{
    // 업데이트 알림 띄우기
    ...
    console.log("exception caught @ showUpdateNotification:", err);
  }
}

function handleTodayHumor(content){
  ...
  try{
    ...
    console.log("exception caught @ handleTodayHumor:", err);
  }
}

function highlightZeroLevel(myLocalStorage){
  try{
    // 0렙 강조
    ...
    console.log("exception caught @ highlightZeroLevel:", err);
  }
}

function drawNoalaIcon(myLocalStorage){
  try{
    ...
    console.log("exception caught @ drawNoalaIcon:", err);
  }
}

function drawInfoImmediately(myLocalStorage){
  try{
    // 잉여력 표시 기능
    if (JSON.parse(myLocalStorage["enabled_ingyeo"])) {
      console.log("drawInfoImmediately -> Temporarily disabled");
    //   var ingyeoNodes = [];
    //   var ingyeoReg = new RegExp(/포인트: ([\d,]+) 잉여력 \((\d+)\%\)/);
    //   var ingyeoLevelReg = new RegExp(/레\s+벨: (\d+)\/30/);
    //   //*[@id="content"]/div[1]/form/table/tbody/tr[12]/td[3]/div
    ...
    console.log("exception caught @ drawInfoImmediately:", err);
  }
}
...
function forwardingNotification(){
  try{
    ...
    console.log("exception caught @ forwardingNotification:", err);
  }
}

function installShortcuts(myLocalStorage){
  try{
    // 단축키 기능
    ...
    key(myLocalStorage["keybinding_advanced_scrap"], function () {
      if (JSON.parse(myLocalStorage["enabled_advanced_scrap"])) {
        var n = noty({
          layout: 'topLeft', type: "warning", text: '고급스크랩: 아직 숙성이 되지 않은 기능입니다. 통수 ㅍㅌㅊ?',
        });
    ...
    console.log("exception caught @ installShortcuts: ", err);
  }
}
...
function initialize(){
  ...
  chrome.extension.sendRequest({ method: "getLocalStorage" }, function (myLocalStorage_) {
    ...
    showUpdateNotification(myLocalStorage);

    var content = document.getElementById("content");
    handleTodayHumor(content);
    highlightZeroLevel(myLocalStorage);
    drawNoalaIcon(myLocalStorage);
    drawInfoImmediately(myLocalStorage);
    forwardingNotification();
    installShortcuts(myLocalStorage);
  });
}
